...
export class ApiClient {
  ...
  /**
   * 获取按周拆分的快照（新格式 snap_{projId}_{weekKey}）
   */
  async getWeeklySnapshot2(snapKey: string) {
    return proxy('get', snapKey, null)
  }
  ...
  // snapshot合法字段白名单
  static VALID_SNAP_FIELDS = new Set([
    'status', 'stage', 'coreOutput', 'coreOutputItems', 'coreAction',
    'risk', 'crossDept', 'decision', 'next', 'incident', 'knowledge',
    '_ts', '_fieldTs', '_savedWk', '_updatedBy', '_updatedAt'
  ])

  static cleanSnapshot(snap: any): any {
    if (!snap || typeof snap !== 'object') return snap
    const clean: any = {}
    for (const [k, v] of Object.entries(snap)) {
      if (ApiClient.VALID_SNAP_FIELDS.has(k)) clean[k] = v
    }
    return clean
  }
  ...
  async fullSync(localData: any): Promise<ApiResponse> {
    ...
    if (localData.weeks !== undefined && localData._projId !== undefined) {
      ...
      // 按周拆分写入：snap_{projId}_{weekKey}
      for (const [wk, wkData] of Object.entries(localData.weeks)) {
        if (!wk.match(/^20\d\d-W\d+$/)) continue
        const cleaned = ApiClient.cleanSnapshot(wkData)
        if (cleaned && Object.keys(cleaned).length > 0) {
          promises.push(proxy('set', `snap_${projId}_${wk}`, cleaned))
        }
      }
    }
