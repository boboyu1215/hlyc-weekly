...
import { pushMentionsForProject } from '@/services/mention';
...
export const useSyncStore = defineStore('sync', () => {
  ...
  async function pushToServer() {
    ...
    try {
      ...
      // 第一步：先同步基础数据（projects / users / activity），不依赖 _projId
      const baseResult = await apiClient.fullSync({
        projects: localData.projects,
        users: localData.userRegistry,
        activity: localData.activityLog
      });
      if (!baseResult || baseResult.success === false) {
        throw new Error((baseResult && baseResult.error) || '上传基础数据失败');
      }

      // 第二步：weeks 数据按项目拆分，逐项目 fullSync
      const rawWeeks = (localData.weeks || {}) as Record<string, any>;
      const byProject: Record<string, Record<string, any>> = {};

      const sampleKey = Object.keys(rawWeeks)[0];
      const isWeekKeyed = !!sampleKey && /^\d{4}-W\d{1,2}$/.test(sampleKey);

      if (isWeekKeyed) {
        // 方式 A：按周聚合 → 转为按项目聚合
        for (const [wk, projMap] of Object.entries(rawWeeks)) {
          if (!projMap || typeof projMap !== 'object') continue;
          for (const [pidStr, snap] of Object.entries(projMap as Record<string, any>)) {
            if (pidStr.startsWith('_')) continue;
            if (!byProject[pidStr]) byProject[pidStr] = {};
            byProject[pidStr][wk] = snap;
          }
        }
      } else {
        // 方式 B：已是按项目聚合，原样使用
        for (const [pidStr, val] of Object.entries(rawWeeks)) {
          if (pidStr.startsWith('_')) continue;
          byProject[pidStr] = val as Record<string, any>;
        }
      }
      ...
      let okCount = 0;
      let failCount = 0;
      for (const [pidStr, projWeekData] of Object.entries(byProject)) {
        const projId = Number(pidStr);
        if (isNaN(projId)) continue;
        if (!projWeekData || Object.keys(projWeekData).length === 0) continue;
        ...
        try {
          const result = await apiClient.fullSync(projPayload);
          if (result && result.success !== false) {
            okCount++;
          } else {
            failCount++;
            console.error(`[pushToServer] 项目 ${projId} 写入失败:`, result?.error);
          }
        } catch (e) {
          failCount++;
          console.error(`[pushToServer] 项目 ${projId} 异常:`, e);
        }
      }
      ...
      console.log(`[pushToServer] 完成：基础数据 ✓，项目快照 ${okCount}/${okCount + failCount}`);

      if (failCount > 0 && okCount === 0) {
        throw new Error('所有项目快照写入失败');
      }
      return true;
    } catch (error) {
      console.error('[pushToServer] 失败:', error);
      setSyncStatus('err');
      throw error;
    }
  }
      ...
      for (const projId of projectIds) {
        try {
          ...
          for (const wk of weekKeys) {
            ...
            if (!local[wk] || Array.isArray(local[wk])) local[wk] = {}
...
export async function saveAndSync(
  ...
  try {
    ...
    const local = storage.loadWeeks();
    if (!local[weekKey]) local[weekKey] = {};
    local[weekKey][projId] = cleaned as any;
    storage.saveWeeks(local);
    ...
    try {
      ...
      const project = projectStore.getProject(projId);
      await pushMentionsForProject(project, snapData, submitter);
    } catch (e) {
      console.error('[mention] 推送异常（不阻塞主流程）', e);
    }
