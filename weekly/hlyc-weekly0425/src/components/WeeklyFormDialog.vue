...
// 更新列表项文本
function updateItemText(field: 'risk' | 'coreAction' | 'decision' | 'crossDept', idx: number, text: string) {
...
// 更新列表项日期
function updateItemDate(field: 'risk' | 'coreAction' | 'decision' | 'crossDept', idx: number, date: string) {
  const items = form.value[field] as TaskItem[];
  items[idx] = { ...items[idx], dueDate: date };
}

// 回车增加新行
function handleKeydown(e: KeyboardEvent, field: 'risk' | 'coreAction' | 'decision' | 'crossDept', idx: number) {
...
// 删除行
function deleteItem(field: 'risk' | 'coreAction' | 'decision' | 'crossDept', idx: number) {
  const items = [...(form.value[field] as TaskItem[])];
  items.splice(idx, 1);
  if (items.length === 0) {
...
function handleSave() {
  ...
  // 日期必填验证（匹配旧系统 forms.js 的 _showSaveAlert）
  ...
  for (let i = 0; i < coreActions.length; i++) {
    ...
    if (item.text && item.text.trim() && !item.dueDate) {
      // 震动弹窗提示，不关闭弹窗
      shakeForm();
      alert(`第 ${i + 1} 项「${item.text}」未设置完成时间，请先设置完成时间后再保存`);
      return;
    }
  }
  ...
  const cleanedForm = {
    ...
    coreAction: (form.value.coreAction as TaskItem[]).filter(item => item.text.trim()),
...
<template>
  <div v-if="show" class="modal-overlay" @click.self="handleCancel">
    <div class="modal" :class="{ 'form-shake': formShaking }" style="max-width: 700px;">
      <!-- 头部 -->
      <div class="modal-header">
        <div>
          <div class="modal-title">更新本周状态</div>
      ...
      <div class="modal-body">
        ...
        <div class="ff">
          <label class="ff-label">上周完成的主要工作</label>
          <textarea
            v-model="form.coreOutput"
            class="auto-h"
            placeholder="上周完成的主要成果"
            @input="autoHeight"
          ></textarea>
        </div>
